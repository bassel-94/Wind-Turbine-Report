...
p2<-ggplot(df.test, aes(x=Windspeed, y=Production_power)) +
  ...
  ggtitle("Semi filtered power production data with respect to the windspeed") +
  theme(plot.title = element_text(size = 10, face = "bold", hjust = 0.6),
        legend.title = element_text(size = 8, face = "bold"),
        legend.text = element_text(size = 6),
        axis.text=element_text(size=6),
...
```{r}
dim(df_model)[1] - dim(data_clean_3)[1]
```
...
```{r gof_gen,fig.cap="\\label{fig:gof_gen}Observed Vs Predicted values of Gen_RPM_Avg", fig.height=3, fig.width=8, warning=FALSE}
...
p1 = ggplot(g1, aes(x=predicted, y=observed)) + 
  geom_point(alpha = 0.1, size = 0.7) + theme_bw() +
  ggtitle("Obs. vs pred. values in validation set Gen_RPM_Avg") + 
...
p2 = ggplot(g2, aes(x=predicted, y=observed)) + 
  geom_point(alpha = 0.1, size = 0.7) + theme_bw() +
  ggtitle("Obs. vs pred. values in training set Gen_RPM_Avg")+ 
...
```{r hist, warning=FALSE, fig.height=3, fig.width=4}
res = (g1$observed - g1$predicted) %>% 
  as_tibble()

ggplot(res, aes(x=value)) + 
  geom_histogram(color="white", bins = 70) + 
  theme_bw() + 
  ggtitle("Distribution of the residuals on the validation set") +
  xlim(c(-70,70)) +
  theme(plot.title = element_text(size = 10, face = "bold", hjust = 0.6), 
        legend.title = element_text(size = 8, face = "bold"),
        legend.text = element_text(size = 6),
...
```{r gen_test,fig.width=5, fig.height=3, warning=FALSE,fig.cap="\\label{fig:gen_test}Observed Vs Predicted Gen_RPM_Avg values in test set"}
#-- build data frames of predicted and observed values of train and test data
...
```{r gen_anom, fig.width=8, fig.height=3,fig.cap="\\label{fig:gen_anom}Predicted anomalies by thresholding on the Generator"}
...
```{r gear_anom, fig.width=8, fig.height=3,fig.cap="\\label{fig:gear_anom}Predicted anomalies by thresholding on the Gearbox"}
